...
while True:
    ...
    for i in movie:
        for page in range(1, 200):
            try:
                data = requests.get("https://movie.daum.net/moviedb/grade?movieId=" + i + "&type=netizen&page=" + str(page))
            except:
                continue
            ...
            rate = []
            content = []
            reviews = soup.findAll("div",{"class":"review_info"})
            for i,r in enumerate(reviews):
                rate.append(int(r.find("em", {'class': 'emph_grade'}).text))
                re = r.find("p", {'class': 'desc_review'}).text
