...
class EMB_ATT_LSTM_ATT_ver2(nn.Module):
    ...
    def sentiment_net(self, lstm_outputs):
        ...
        print(sig_output[:,:,0].unsqueeze(1).repeat(1,1,768).shape)
        emb_result = self.sentiment_embedding(zeros) * sig_output[:,:,0].repeat(1,768) + self.sentiment_embedding(ones) * sig_output[:,:,1].repeat(1,1,768)
        senti_output = lstm_outputs + emb_result
        return senti_output
