...
class BASEELECTRA_COS2_STAR_NEG_EMB(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels, token_type_ids):
        ...
        if len_p <= 1 and len_n > 1:
            ...
            if len_n == 0:
                result = ((loss1, torch.tensor(0), 0.25 * loss3_p, torch.tensor(0)), outputs)
            else:
                result = ((loss1, torch.tensor(0), 0.25 * loss3_p, torch.tensor(0)), outputs)
        else:
            result = ((loss1, loss2 * 0.75,
                       float(len_p) / (len_p + len_n) / 4 * loss3_p, float(len_n) / (len_p + len_n) / 4* loss3_n),
                      outputs)
...
class ENSEMBLE_MODEL_last(nn.Module):
    def __init__(self, model_type, model_name_or_path, config):
        super(ENSEMBLE_MODEL_last, self).__init__()
        self.emb = MODEL_ORIGINER[model_type].from_pretrained(
            model_name_or_path,
            config=config)
...
MODEL_LIST = {
    ...
    "ENSEMBLE_MODEL_last":ENSEMBLE_MODEL_last,
