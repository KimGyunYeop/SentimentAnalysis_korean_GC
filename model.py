...
class KOSAC_LSTM_ATT_DOT_ML(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels, token_type_ids,polarity_ids, intensity_ids):
        ...
        outputs = self.dropout(outputs)
        outputs = self.out_proj(outputs)
        print(soft_attn_weights.squeeze().size())
        print((torch.abs(polarity_ids)+torch.abs(intensity_ids)).size())
        att_label = F.softmax(torch.abs(polarity_ids)+torch.abs(intensity_ids))
        print(att_label.size())
        loss_fct = nn.CrossEntropyLoss()
        loss_att = nn.CrossEntropyLoss()
        loss = (loss_fct(outputs.view(-1, 2), labels.view(-1)) + loss_att(soft_attn_weights.squeeze(),att_label))/2
